...
2. Ask user to send /start to the new bot (required for new bots), then send any message, then get chat_id:
   ...
   ```
3. Create the agent. Prefer a Nova Cortex specialist template if it matches the role:
   - `nova-cortex-cfo` (finance/forecasting)
   - `nova-cortex-marketer` (campaigns/content)
   - `nova-cortex-ops` (processes/automation)
   - `nova-cortex-research` (intel/analysis)
   - Otherwise fall back to the generic `agent` template:
   `cortextos add-agent <name> --template <template-name>`
