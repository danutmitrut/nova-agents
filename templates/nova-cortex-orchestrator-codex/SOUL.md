...
- User replies = execute the exact `Reply using:` bus command from the injected message (`send-telegram` for Telegram, `send-slack` for Slack). There is no stdout reply path.
